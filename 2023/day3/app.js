...
const example = `......124..................418.......587......770...........672.................564............................438..........512......653....
...
function checkArr(row, col) {
  if(row >= 0 && col >= 0 && row < arr.length && col < arr[row].length) {
    if(!arr[row][col].match(/^\d+|\d+\b|\d+(?=\w)/g) && arr[row][col] != ".") {
        return true;
    }
  }
  return false;
...
function getFullValue(row, col) {
  let result = ""

  // console.log("Row:", row, "Col:", col);
  for(let i = col; i >= 0; i--) {
    if(arr[row][i].match(/^\d+|\d+\b|\d+(?=\w)/g)) {
      result = arr[row][i] + result;
    } else  {
      i = -1;
    }
  }
  for(let i = col + 1; i < arr[row].length; i++) {
    // console.log(i)
    if(arr[row][i].match(/^\d+|\d+\b|\d+(?=\w)/g)) {
...
let lastNumberCounted = 0;

let response = arr.reduce((arrAgg, currRow, row) => {
  lastNumberCounted = 0;
  return arrAgg + currRow.reduce((colAgg, currCol, col) => {
    let add = 0;
    let stripped = currCol.match(/^\d+|\d+\b|\d+(?=\w)/g);
    if(stripped) {
      if((checkArr(row-1, col-1) || 
        checkArr(row, col-1) || 
        ...
        let valueToCheck = +getFullValue(row, col);
        // console.log("Row:", row, "Col:", col);
        // console.log(valueToCheck);
        if(lastNumberCounted != valueToCheck) {
          add = valueToCheck;
          lastNumberCounted = valueToCheck;
        }
      }
    } else {
      lastNumberCounted = 0;
...
function checkNumber(row, col) {
  if(row >= 0 && col >= 0 && row < arr.length && col < arr[row].length) {
    if(arr[row][col].match(/^\d+|\d+\b|\d+(?=\w)/g)) {
      return getFullValue(row, col)
    }
  }
  return 1;
}
...
for (let row = 0; row < arr.length; row++){
  for(let col = 0; col < arr[row].length; col++) {
    if(arr[row][col] === "*") {
      let one = checkNumber(row-1, col-1);
      let two = checkNumber(row-1, col);
      let three = checkNumber(row-1, col+1);
      if(one === two && one === three) {two = 1; three = 1;}
      if(one === two) two = 1;
      if(two === three) three = 1;

      let four = checkNumber(row, col-1);
      let six = checkNumber(row, col+1);
      let seven = checkNumber(row+1, col-1);
      let eight = checkNumber(row+1, col);
      let nine = checkNumber(row+1, col+1);
      if(seven === eight && seven === nine) {eight = 1; nine = 1;}
      if(seven === eight) eight = 1;
      if(eight === nine) nine = 1;
      ...
      if(myarr.length === 2)
      result += one*two*three*four*six*seven*eight*nine;
    }
  }
}
