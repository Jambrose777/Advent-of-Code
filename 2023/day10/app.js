...
let connections = field.map(row => row.map(entry => ({entry})));

function getValue(row, col) {
  if(row >= 0 && col >= 0 && row < connections.length && col < connections[row].length) {
...
// figure out which direction to go from S
let northValue = getValue(loopCoordinates[0].i - 1, loopCoordinates[0].j);
let eastValue = getValue(loopCoordinates[0].i, loopCoordinates[0].j + 1);
let southValue = getValue(loopCoordinates[0].i + 1, loopCoordinates[0].j);
let westValue = getValue(loopCoordinates[0].i, loopCoordinates[0].j - 1);
let SStartDirection;
let SEndDirection;
if(northValue === '|' || northValue === '7' || northValue === 'F') {
  loopCoordinates.push({i: loopCoordinates[0].i - 1, j: loopCoordinates[0].j})
  SStartDirection = 'north';
} else if(eastValue === '-' || eastValue === 'J' || eastValue === '7') {
  loopCoordinates.push({i: loopCoordinates[0].i, j: loopCoordinates[0].j + 1})
  SStartDirection = 'east';
} else if(southValue === '|' || southValue === 'J' || southValue === 'L') {
  loopCoordinates.push({i: loopCoordinates[0].i + 1, j: loopCoordinates[0].j})
  SStartDirection = 'south';
} else if(westValue === '-' || westValue === 'F' || westValue === 'L') {
  loopCoordinates.push({i: loopCoordinates[0].i, j: loopCoordinates[0].j - 1})
  SStartDirection = 'west';
}

let condition = true;
//find full path of loop.
while(condition) {
  let currentCoordinates = loopCoordinates[loopCoordinates.length - 1];
  let currentValue = getValue(currentCoordinates.i, currentCoordinates.j);

  let previousCoordinates = loopCoordinates[loopCoordinates.length - 2];
  let previousDirection;
  if(previousCoordinates.i === currentCoordinates.i) {
    if(previousCoordinates.j > currentCoordinates.j) {
      previousDirection = 'east'
    } else {
      previousDirection = 'west'
    }
  } else if (previousCoordinates.i > currentCoordinates.i) {
  ...
  if(currentValue === 'S') {
    condition = false;
    SEndDirection = previousDirection;
  } else if(previousDirection == 'east') {
    if(currentValue === '-') {
      loopCoordinates.push({i:currentCoordinates.i, j:currentCoordinates.j-1})
    } else if(currentValue === 'L') {
      loopCoordinates.push({i:currentCoordinates.i-1, j:currentCoordinates.j})
    } else {
      // if(currentValue === 'F') {
      loopCoordinates.push({i:currentCoordinates.i+1, j:currentCoordinates.j})
    }
  ...
  } else if(previousDirection == 'north') {
    if(currentValue === '|') {
      loopCoordinates.push({i:currentCoordinates.i+1, j:currentCoordinates.j})
    } else if(currentValue === 'J') {
      loopCoordinates.push({i:currentCoordinates.i, j:currentCoordinates.j-1})
...
console.log(`Part 1: ${(loopCoordinates.length - 1) / 2}`)

// calculate the correct value for S.
let sValue;
if(SStartDirection === 'north') {
  if (SEndDirection === 'east') {
    sValue = 'L'
  } else if (SEndDirection === 'south') {
    sValue = '|'
  } else {
    sValue = 'J'
  }
} else if(SStartDirection === 'east') {
  if (SEndDirection === 'north') {
    sValue = 'L'
  } else if (SEndDirection === 'south') {
    sValue = 'F'
  } else {
...
} else if(SStartDirection === 'south') {
  if (SEndDirection === 'north') {
    sValue = '|'
  } else if (SEndDirection === 'east') {
    sValue = 'F'
  } else {
    sValue = '7'
  }
} else if(SStartDirection === 'west') {
  if (SEndDirection === 'north') {
    sValue = 'J'
  } else if (SEndDirection === 'east') {
    sValue = '-'
...
// strip the field into only the loop values
let loopOnly = field.map((row, i) => row.map((entry, j) => (loopCoordinates.find(coors => coors.i == i && coors.j == j) ? (entry == 'S' ? sValue : entry) : '.')));

// calculate everything inside the loop
for(let i = 0; i < loopOnly.length; i++) {
  let outside = true;
  let previousWall;
  for(let j = 0; j < loopOnly[0].length; j++) {
    if (loopOnly[i][j] === '.') {
      if(outside) {
        loopOnly[i][j] = 'O'
      } else  {
    ...
    } else if (loopOnly[i][j] === '|') {
      outside = !outside;
    } else if (!previousWall) {
      if(loopOnly[i][j] === 'F' || loopOnly[i][j] === 'L') {
        previousWall = loopOnly[i][j];
      }
    } else {
      if(loopOnly[i][j] === '7' ||  loopOnly[i][j] === 'J') {
        if(loopOnly[i][j] === '7') {
          if(previousWall === 'F') {
            //nothing
          } else {
